...
HEADLINE: list[dict] = [
    ...
    # --- what makes a null strong or weak? (docs/paper/review/null_mechanism.md) ---
    {"path": "results/review/null_mechanism.json",
     "script": "scripts/null_mechanism.py", "tier": "cpu",
     "claims": "pre-registered search (candidates, predicted directions and adjudication "
               "rule committed at ffbec98 with the script UNRUN) for a computable property "
               "of benchmark metadata that predicts covariate-null strength. HEADLINE IS A "
               "NULL RESULT: of thirteen declared candidates eleven were computable, none "
               "orders the five benchmarks, and two "
               "of the most intuitive -- effective dimensionality and sampling density per "
               "dimension -- order them with the sign REVERSED. At n=5 a perfect ordering "
               "would be p=0.0167 exact-permutation, p=0.167 corrected, so nothing could "
               "have reached significance; that was committed before the run. Harmonises "
               "the input table first and it REORDERS: AirfRANS 0.932 was a Spearman not an "
               "R2 and becomes 0.6833, DrivAerML 0.973 (outer-decile split) becomes 0.9598, "
               "DrivAerNet++ 0.737 (595 test designs have no parameters at all) becomes "
               "0.8248; AhmedML 0.6842, WindsorML 0.1045. Two positive findings. (1) A "
               "single null number conflates two different failures: AirfRANS 0.6833 and "
               "AhmedML 0.6842 are the same number with opposite causes -- flexible ceiling "
               "0.946 vs 0.775 -- so the pair (R2_linear, R2_flexible) must be reported. "
               "(2) WindsorML's counterexample status is most likely a metadata defect: its "
               "paper lists 7 design parameters, geo_parameters_all.csv publishes 6 "
               "(ratio_length_front_rear absent, verified against the live HuggingFace "
               "file) plus a frontal_area column that is 97.1%% explained by clearance "
               "alone. Accounting gap >= 0.538 against the published MGN bound; the "
               "pre-registered leave-one-parameter-out calibration was FALSIFIED, showing "
               "one design parameter is worth up to 0.705 (AirfRANS alpha) or 0.567 "
               "(DrivAerML Vehicle_Width), so one withheld DOF is quantitatively "
               "sufficient. Sparsity excluded (P2 held: Windsor flexible learning curve "
               "plateaus at 0.252). Controls: n-matched at n=200 preserves the ordering "
               "exactly; reference-area convention moves AhmedML by 0.345 and reorders the "
               "table; null strength varies within a benchmark by up to 0.36 across force "
               "channels. AhmedML regime-crossing prediction P1 FALSIFIED on its committed "
               "0.05 threshold (gain 0.0489) though the critical-slant drag peak is present "
               "at 32-38 deg exactly as predicted. Metadata only; no field data, no GPU."},
...
def main(argv=None) -> int:
    ...
    # newline="\n": the repo stores result JSON with LF (.gitattributes: eol=lf); on
    # Windows the default text-mode write emits CRLF, and while git normalises that to
    # LF on commit, a `--check` run against the pre-commit working tree bytes would then
    # disagree with what a fresh checkout hashes. Write LF explicitly so this file's own
    # bytes are already what git will store.
