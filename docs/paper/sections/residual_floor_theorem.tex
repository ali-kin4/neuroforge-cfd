...
The section is organised around the paper's three verbs. \textbf{Certify: no}, and this is
the leg the theory settles---\autoref{thm:consistency-floor} shows the floor survives the
continuum limit, and \autoref{thm:residual-floor} shows the truth is neither a minimiser
nor a stationary point, so no threshold on the residual norm implies a bound on the error.
...
trained backbone's over-smoothed prediction (the dropout-FNO of
\texttt{checkpoints/certificates\_deq.pt}, distinct from the Transolver headline) sits
\emph{below} the truth floor in $160/200$ cases. The floor is not an artifact of the
...
corrector on AirfRANS that the resulting monitor can rank predictions but cannot certify
them and cannot be descended.
