...
(median $0.133$) in the whole-grid norm named above, while the uniform-freestream field
gives $\|R_h(u_\infty)\|=0$ in every case---the objective strictly prefers a physically
wrong field to the truth (\texttt{results/certificates/residual\_floor\_realdata.json}). A
...
backbone and we rest no claim on it. It is worth naming the mechanism correctly, since the
natural guess is wrong: the dropout-FNO is not a \emph{smoother} field---its
velocity-gradient energy is $1.75\times$ the truth's over the fluid (and $1.96\times$ in a
near-body band), against $1.00\times$ for Transolver. What distinguishes it is that its
monitored residual is nearly case-\emph{independent} (spread $0.034$ versus the truth's
$0.162$, the Transolver's $0.161$), so it neither tracks case difficulty nor clears the
floor. By \eqref{eq:rf-decomp} a prediction can fall below the floor only when its error is
systematically anti-aligned with $r^\star$; unbiased error of any magnitude raises the
monitored residual, which is why the accurate backbone sits above it. What does not depend
