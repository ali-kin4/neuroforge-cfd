...
%   zhang2026phymgn.
%
% Structure: Theorem 1 (consistency floor, nonzero continuum limit, with the exact
%   2-D identity |T| = sqrt2 ||S||_F |grad nu_t|); Theorem 2 (misplaced minimum,
%   displaced minimiser, local divergence); Proposition 1 (kernel: counting bound,
%   exact solid invisibility, diagonal nu_t block, Nyquist/Rhie-Chow mode).
% The closure-omission term is ~4.5% of the floor and full stress repair moves it
%   <0.1% (floor_resolution_decomposition.json) -- kept in proportion throughout.
% ---------------------------------------------------------------------------
...
certificate is $6.5$--$7.9\times$ the quantity being certified (\autoref{sec:conformal}).
...
(we reserve $L$ for the Jacobian below). The scalar we write $\|R_h(\cdot)\|$ in this section is the deployed one,
...
blind to inconsistency. The gate certifies the discretisation; it cannot certify the
operator, and we state the blind spot alongside it.
...
\paragraph{Scope of legs (iii) and (iv).}
Legs (iii) and (iv) of \autoref{thm:residual-floor} are statements about the
\emph{linearised} objective at $u^\star$ and
...
The undetectable subspace is not a matter of a few isolated null modes. It is large, and
its size is provable by counting.
...
($D_x$ reaches $(i\pm1,j)$ and $D_y$ reaches $(i,j\pm1)$; the compact $\Delta_h$ reaches both pairs).
...
\subsection{Ranking survives; certification survives only to a floor-limited width}

\paragraph{What the floor costs certification, and why it does not cost triage.}
Certification \emph{from the residual norm alone} asks for a map from $\|R_h(\hat u)\|$ to
a bound on $\|\hat u-u^\star\|$.
The floor does \emph{not} make such a bound invalid---split conformal returns one
whatever the score, and ours attains $0.901$--$0.903$ coverage against a $0.90$ target.
What the floor leaves is a wide bound: $6.5$--$7.9\times$ the median drag error it
certifies and $1.7\times$ the median field error (\autoref{sec:conformal}). We do
\emph{not} attribute that width to the floor. Because $u^\star$ itself is rejected at
every threshold below $\|r^\star\|$ (\autoref{thm:residual-floor}(i)) and the sublevel
sets are unbounded in the directions of \autoref{prop:kernel}, a fixed part of every
score is present at zero error---the median truth-to-prediction residual ratio on the
deployed arm is $0.864$, so $86\%$ of a typical score is floor---but removing the floor
\emph{exactly}, via the oracle field difference
$\sigma'=\|R_h(\hat u)-R_h(u^\star)\|$ (a difference of norms is not a valid
decomposition, and $\|R_h(\hat u)\|<\|r^\star\|$ on $2.5$--$4.0\%$ of cases), makes the
bound \emph{wider}, $7.4\times\!\to\!11.3\times$ on $3/3$ seeds
(\texttt{results/review/floor\_subtraction\_gate.json}). Floor subtraction improves the
score's \emph{ranking} (Spearman $0.616/0.592/0.626\to0.681/0.703/0.601$) and degrades
its use as a conformal \emph{scale}: the width tracks the dispersion of the
nonconformity ratio $E/\sigma$ ($Q_{0.9}/\mathrm{median}$ of $15.1$ for an uninformative
score, $6.3$ for the monitor, $10.5$ after floor subtraction). The width is the heavy
tail of the drag-error distribution, not the floor---the marginal interval with no
monitor at all is $17.2\times$, so the monitor already buys $2.3\times$---and since the
oracle $r^\star$ dominates any predictor of it, no learned floor correction can improve
this certificate either. An interval $1.7\times$ the median error is usable; one
$8\times$ the median drag error is not a design tool. And refinement does not recover
it: the floor does not decay under refinement, and the reconstruction the ladder
differentiates is fixed independently of $h$, so the fitted order is a crossover slope
rather than the exponent of a scaling law (\autoref{sec:floor_ladder}). The guarantee is
marginal and rests on exchangeability of the calibration and test cases, here a random
split of one pooled AirfRANS partition; it says nothing about the out-of-distribution
regime in which a certificate is most wanted. Ranking asks something much
...
\paragraph{An apparent contradiction, and its resolution.} On the weak dropout-FNO
backbone the prediction sits \emph{below} the floor in $80\%$ of cases
($\|R_h(\hat u)\|=0.114$ against $\|r^\star\|=0.192$), which places it in the regime the
decomposition calls floor-dominated---yet $\rho=0.61$ is measured there. We scope that
figure deliberately: it does \emph{not} generalise. On the deployed Transolver the sign
reverses, the prediction sits \emph{above} the floor at $0.212$--$0.215$, and the
inversion rate is only $2.5$--$7.0\%$ across three seeds
(\texttt{results/certificates/transolver\_inversion.json}). The contradiction is therefore
only apparent on one backbone, but it is worth resolving because the resolution is the
same in both. The two are statements about
...
\emph{below} the truth floor in $160/200$ cases. That is the flattering number, and it does
not generalise: on the
...
backbone and we rest no claim on it. The obvious explanation does not survive
measurement: the inversion is \emph{not} over-smoothing. The
...
manufactured analytic solution converges on the same monitor at order $2.02\pm0.05$ per
case ($2.06$ fitted on the rung means) and, pushed through the identical rasteriser, still
\emph{falls} with refinement (fine/coarse $0.65$, rising in only $3/16$ cases) where the
real truth's rises; and a $C^1$ re-rasterisation \emph{raises} rather than lowers the floor
(\autoref{sec:floor_ladder}). The manufactured field is a potential flow, so the monitor is
structurally blind to the inconsistency on it; these controls bound the pipeline's own
error, and are not evidence that the monitored operator is consistent. Those band-restricted magnitudes are a different scoring
...
$\lambda\ge0$ \emph{at that resolution}. This leg is resolution-contingent and we claim no
more. The framework's proximity weight decays over $3\min(dx,dy)$ and so
...
$362^2$, and $-2.8\%$ at $512^2$ in a $16$-case run, where it has crossed. A $24$-case run
over $128^2/256^2/512^2$ agrees on the collapse but still has the truth above by $1.5\%$ at
the finest rung (\texttt{results/floor\_resolution\_ladder.json}); a ${\pm}2\%$ disagreement
between two subsets at a margin that has already collapsed to ${\sim}2\%$ is subset noise,
and we take the cautious reading. Where it crosses, the crossover
weight
$\lambda^\star=\|R_h(u^\star)\|^2/(\overline{r_{bc}^2}(u_\infty)-\overline{r_{bc}^2}(u^\star))$
has median $599$ at $512^2$, the rung where it crosses. Since the deployed trust map uses
...
in physical units. (H2) therefore holds here for a reason that is \emph{not} the deployed
grid's resolution: the floor does not decay under refinement, the individually
grid-converged convective and pressure-gradient terms
($0.183\!\to\!0.199$ and $0.180\!\to\!0.181$) do not cancel under a discretisation
different from the one that produced the labels, and restoring the omitted stress
divergence moves the floor by under $0.1\%$. We do \emph{not} attribute the floor's
\emph{growth} to operator provenance. Holding $h$ fixed and thinning the source point
cloud by a factor of eight raises the floor in $16/16$ cases on every band and at both
rungs tested (fitted $\mathrm{d}\log\|r^\star\|/\mathrm{d}\log s=+0.55\pm0.27$;
...
measurement we have separates the two contributions. One further bound: the measured
...
representation error, so the measured floor mixes the theorem's term with that error. The
decimation experiment above bounds that mixing from below: at fixed $h$ the
representation term is large enough to move the measured floor by $80\%$. That
...
form, elementary---the pressure gauge above. Our contribution is threefold.
\textbf{(a)} A \emph{proof} that this floor has a
...
accounted for. \textbf{(b)} An exact characterisation of the undetectable
...
entries, and the Nyquist pressure mode. \textbf{(c)} The demonstration on a learned neural-operator CFD
