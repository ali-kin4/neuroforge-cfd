...
across seeds (\texttt{results/review/functional\_audit\_gate\_analysis.json}), so the
truth is the best-scoring field on more than $93\%$ of them. The inversion is largely a
property of an over-smoothed backbone---a smooth field has small derivatives---and we do
not rest any claim on it. What does not depend on the backbone is the floor itself, which
no model enters. The floor is not an artifact of the
