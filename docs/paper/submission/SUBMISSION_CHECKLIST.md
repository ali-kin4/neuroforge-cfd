...
## Blocking — in this order, because the release is irreversible and must come last

1. **Title.** It was changed without the author's sign-off in a late editing round (from
   the approved *"Near the wall, AirfRANS measures the coordinate system, not the model"*).
   Keep or revert **before** anything is minted: the title lives in both wrappers, the
   letter, `.zenodo.json`, `pdftitle` and the arXiv notes.
2. **What the cited snapshot contains.** 33 tracked files on this branch (3 already on
   `main`) name the CMAME and JCP desk rejections: `docs/paper/review/*`, this checklist's
   history table, the letter's notes to self, the arXiv notes, PLANS/GOALS, two scripts.
   The repository is public, so the letter's non-disclosure rests on a premise ("not
   discoverable") that is already false for anyone who browses it. Decide: strip those
   files from the tree before tagging, or disclose the rejections in one line of the
   letter. Either is defensible; leaving both as they are is not.
3. **Vitae and title-page addresses.** Fill the `[[FILL]]` markers in `vitae.md`
   (positions, degrees, the second author's interests — not recorded anywhere here), then
   rebuild until the packager prints `READY`. The guide also asks for each affiliation's
   **full postal address, including the country**; the title page has city and country
   only. Add street and postcode to the `\affiliation` fields if you have them; they
   were not invented here.
4. **Declaration of interest via Elsevier's tool.** [V-JOCS 2026-09-17]: *"The
   declarations tool should always be completed."* Select **"I have nothing to declare"**,
   save as .docx, upload at "attach files". Deliberately **not** generated here.
5. **Publish release `v1.1.0` — last, once 1–4 are settled.** The manuscript and letter
   say the snapshot matching the paper is release `v1.1.0` at Zenodo (concept DOI
   10.5281/zenodo.21277928). Until it exists the DOI resolves to **v1.0.5 (2026-08-20),
   which archives the old trust-layer paper and none of the headline scripts**, and `main`
   is several hundred commits behind. Steps (a minted DOI cannot be retracted):
   fast-forward `main` to this branch; tag `v1.1.0`; publish the GitHub release (the
   Zenodo webhook fires on `release` events; `.zenodo.json` is ready); confirm the concept
   DOI resolves to v1.1.0. Note that the snapshot also carries Paper 2's unpublished
   OpenFOAM work. Then submit.
...
| Declaration of interest | from Elsevier's tool (see Blocking 4) |
...
| Title page | title, authors, affiliations with **full postal address incl. country**, corresponding author's email | [V-JOCS 2026-09-17] | city + country only — **author item** (Blocking 3); addresses not invented |
...
| Research data | Option C: deposit, **cite and link** in the article | [V-JOCS 2026-09-17] | ✓ Zenodo archive cited as a `[software]` reference and linked in "Code and data availability" (true once Blocking 5 is done) |
...
| Vitae | ≤ 100 words per author, editable format | [V-JOCS 2026-09-17] | drafted; **markers to fill** (Blocking 3) |
| Declaration of interest | Elsevier tool, .doc/.docx | [V-JOCS 2026-09-17] | **author step** (Blocking 4) |
...
- [ ] **Blocking 1–5 above are done**, and `build_jocs_package.py` prints `READY`
